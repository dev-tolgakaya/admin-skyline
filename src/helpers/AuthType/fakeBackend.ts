...
import { jobListCandidate, jobsGridData } from "common/data/job";
...
let users = [
  {
    uid: 'dKMzKU5kbzVbbKjPsQgUtttsj3w1',
    username: "admin",
    role: "admin",
    password: "123456",
...
const fakeBackend = () => {
  ...
  //letest trangection
  mock.onGet(url.GET_TRANSCECTION).reply(() => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (visitor) {
          ...
          const filteredvisitorss: any = visitor.filter(
            (msg: any) => msg.id === params.roomId
          );
          resolve([200, filteredvisitorss]);
        } else {
          reject([400, "Cannot get messages"]);
        }
      });
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (statisticsApplications) {
          ...
          const filteredvisitorss: any = statisticsApplications.filter(
            (msg: any) => msg.id === params.roomId
          );
          resolve([200, filteredvisitorss]);
        } else {
  ...
  mock.onPost(new RegExp(`${url.ON_LIKNE_COMMENT}/*`)).reply(config => {
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (productComments) {
          var modifiedProductComments: any = [...productComments];
          ...
          if (commentIdx > -1) {
            if (
              ...
              const replyIdx = (
                ...
                (reply: any) => reply.replyId.toString() === params.replyId.toString()
              );
  ...
  mock.onPost(new RegExp(`${url.ON_ADD_REPLY}/*`)).reply(config => {
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (productComments) {
          const modifiedComments: any = [...productComments];
